...
1、通过Findclass()得到jclass
2、通过GetMethodID()得到jmethodID
3、通过一系列call开头的函数完成调用(如]CallVoidMethod)
...
作用: 通过class linker的FindClass()取得目标类的jclass
...
作用:根据jclass对象和方法名和签名信息取得对应的ArtMethod用于下一步的调用

[jni_internal.cc](http://androidxref.com/8.0.0_r4/xref/art/runtime/jni_internal.cc)
...
[reflection.cc](http://androidxref.com/8.0.0_r4/xref/art/runtime/reflection.cc)

```cpp
JValue InvokeVirtualOrInterfaceWithVarArgs(const ScopedObjectAccessAlreadyRunnable& soa,
                                           jobject obj, jmethodID mid, va_list args) {

  // jmethodId 转换 ArtMethod
...
[dex_file.h](http://androidxref.com/8.0.0_r4/xref/art/runtime/dex_file.h)

```cpp
// 前16个字节固定
struct CodeItem {
  uint16_t registers_size_;            /*使用的寄存器个数 .register*/
  uint16_t ins_size_;                  /*参数个数.paramter*/
  uint16_t outs_size_;                 /*调用其他方法时使用的寄存器个数 outssize指定方法调用外部方法时使用的寄存器个数*/
  uint16_t tries_size_;                /*try/catch个数 */
  uint32_t debug_info_off_;            /*指向调式信息的偏移 */
  uint32_t insns_size_in_code_units_;  /*指令集个数，以2字节为单位 */
  uint16_t insns_[1];                  /*指令集 */
...
// Raw try_item. 总共8字节
struct TryItem {
  uint32_t start_addr_; // try 开始地址
  uint16_t insn_count_; // 指令条数
  uint16_t handler_off_; // 当前tryItem对应handler list 偏移
...
多个catch块放在一起，是一个handler对象，remaining_count表示catch/finally块的个数。如果remaining_count<=0表示最后一个是finally块。如果 remaining count>0表示只有catch块。remaining_count的绝对值表示catch块的个数。如果remaining_count ==0表示没有catch块，只有finally块一个handler块对应一个Tryltem，共同组成try catch/finally块,有多个try-catch/finally就有多个handler块。
...
```cpp
// insns_size_in_code_units_;  /*指令集个数，以2字节为单位 */
...
##### 3、如何遍历dex中的所有函数并完成主动调用

主动加载dex中的所有类需要对dex文件解析，获取dex中的所有类列表，两种解决方案
1、手动解析dex文件
2、直接调用aosp源码中已有的api:getclassNameList即可(需要通过反射一步步获取当前classLoader当中的mCookie)
...
public static void fart() {
    ...
    // 此处遍历主要解决插入classLoader的方案(目前来说都是替换)
...
// 只对app最终的classLoader进行dump，不会包含系统框架层的dex
public static void fartwithClassloader(ClassLoader appClassloader) {
    ...
    for (Method field : DexFileClazz.getDeclaredMethods()) {
        // native String[] getClassNameList(Object cookie);
...
public static void loadClassAndInvoke(ClassLoader appClassloader, String eachclassname, Method dumpMethodCode_method) {
    ...
    try {
        // 此处用loadClass,只加载不初始化。不能使用forName防止某些类初始化后主动退出app
...
因为有些app壳代码还没执行，所以需要休眠



为什么要在performLaunchActivity去启动?

所有activity启动都会进入此方法

某些app没有activity，可以换到handleBindApplication里去启动



各个版本之间有什么区别?

mCookie类型不一样,获取的时候需要根据版本进行判断



被动调用才是真正解决函数抽取问题，主动调用存在调用深度不够问题

可以将函数dump时机，加在解释器执行指令的后面，例如return指令和throw指令(这2个指令时机比较晚)

如果在某些app没有dump下来，就可以尝试这种



对于inline函数frida没办法hook如何解决？

1.修改源代码，增加非inline的函数，打出so库，进行调用
...
对FART进一步增强:
1、增强FART的脱壳能力:解决对抗FART的壳、动态加载的dex的dump和修复
2、控制FART主动调用的范围，让FART更精细化，比如按需进行类甚至是函数的修复。
    ...
    Java.perform(function () {
        ...
        /*        Java.enumerateClassLoaders({
                    onMatch: function (loader) {
                        ...
                        //    public static void fartwithClassloader(ClassLoader appClassloader)
                        try {
                            Java.use("android.app.ActivityThread").fartwithClassloader(loader);
                        } catch (e) {
                            console.log(e);
                        }
                    ...
                    onComplete: function () {
                        console.log("[*]dalvik.system.DexClassLoader Finished heap search")
                    }
                });*/
    })
}
...
if __name__ == "__main__":
    try:
        session = device.attach("packageName") # modify
        print("[Info] Attach success!")
        with open('fart_frida.js') as f: # fileName
            jscode = f.read()
        ...
        script.load()
        content=""
        with open('xxx_classlist_execute.txt','r') as f: # fileName
            content=f.read()
            f.close()
        array=content.split("\n")
        for i in array:
...
注:修改classlist读写权限(777) 和 禁用SELinux (setenforce 0)
...
public static void fartwithClassloader(ClassLoader appClassloader) {
    ...
    String filePath = getPath() +"/classlist.txt";
    while (true) {
        try {
        ...
        try {
            ...
            while ((classname = br.readLine()) != null) {
                ...
                loadClassAndInvoke(appClassloader, classname, dumpMethodCode_method);
            }
            br.close();
            fr.close();
        } catch (Exception e) {
