...
2、每一个 Activity 和 Service 以及 Application 的 Context 是一个新的ContextImpl 对象。 
...
trimToSize：不断地删除LinkedHashMap中队尾的元素，即近期最少访问的，直到缓存大小小于最大值。
...
打点系统是什么？指的是埋点系统吗？如果是埋点的话我觉得你可以参考这几点：
...
使用kotlin 调用一个会返回空的java函数， 是不会提示空安全的，只有加上@nullable之后才会提示。
...
### 7.你知道数据库有哪些，android sqlite存储数据的上限大小是多少？
...
```
sendEmptyMessageDelayed:发送一个空消息，消息体只有what ->sendMessageDelayed(msg, delayMillis);
postDelayed:发送一个runnable消息，runnable会存到Message的callback里
```
...
通过调用windowManageraddView(decor, lp)，将DecorView和WindowManager建立联系。
...
addView
将decorView添加到viewRootImp中，通过viewRootImp对其内部的view进行管理
...
虚拟机栈：虚拟机栈在JVM运行过程中存储当前线程运行方法所需的数据，指令、返回地址。

Java 虚拟机栈是基于线程的。在线程的生命周期中，参与计算的数据会频繁地入栈和出栈，栈的生命周期是和线程一样的。
...
#### LeakCanary 内存泄露框架原理， 除了这种方式监控内存泄露，是否还有其他方式
...
#### OKhttp：socket连接池怎么复用的， 拦截器是怎么实现的，用到了什么设计模式， 对OKhttp 还做了哪些优化

```
在连接拦截器ConnectInterceptor，通过streamAllocation.newSteam查找或者建立一个与请求主机有效的连接，streamAllocation中维护了RealConnection，RealConnection封装了Socket连接池ConnectionPool。

ConnectionPool：
用connections(双端队列)存储所有的连接。
通过线程池来清除超时的RealConnection，线程池和okhttp处理异步时候的线程池一样。
keepAliveDurationNs表示每一个连接keep-alive的时间，默认是5分钟。
maxIdleConnections连接池的最大容量，默认是5个。
transmitters用来保存该连接的transmitter个数，标记该RealConnection有没有在使用中。

通过调用ConectionPool的get方法，遍历双端队列获取可用连接，并调用acquire方法。
如果没有可用连接，创建新连接，放入到双端队列，同时开启线程池清理任务。
cleanupRunnable清理任务中，会不断调用cleanup方法清理线程池，并返回下一次清理的时间间隔，然后进入wait等待。
cleanup方法中，如果空闲连接maxIdleConnections超过5个或者keepalive时间大于5分钟，则将该连接清理掉。
...
#### RXJava： 怎么实现线程切换的， map 和 flatmap 操作符的区别， rx背压问题，怎么解决的，使用RXJava中还遇到哪些问题，怎么解决的
...
#### hashmap：原理，hash冲突，扩容，java 1.8后的修改，哪些map线程安全，怎么实现安全的
...
LiveData原理是观察者模式
observe:添加观察者
首先是判断LifecycleOwner是DESTROYED状态，就直接忽略，不能添加。
LifecycleOwner、observer 组装成LifecycleBoundObserver。
使用putIfAbsent方法observer-wrapper作为key-value添加到观察者列表mObservers中。（意思是只有列表中没有这个observer时才会添加。）
然后对添加的结果进行判断，如果mObservers中已经存在此observer key，但value中的owner不是传进来的owner，就会报错。
“不能添加同一个observer却是不同LifecycleOwner”。如果是相同的owner，就直接return。
最后用LifecycleOwner的Lifecycle添加observer的封装wrapper。

observeForever：和observe()类似，只不过 会认为观察者一直是活跃状态，且不会自动移除观察者。

事件回调：
LifecycleBoundObserver是LiveData的内部类，是对原始Observer的包装，把LifecycleOwner和Observer绑定在一起。
onStateChanged是生命周期状态变化的回调。
如果是DESTROYED状态，则移除观察者。
...
mActiveCount == 0 是指 LiveData 的活跃观察者数量。活跃的观察者数量 由0变为1、由1变为0 会分别调用LiveData的 onActive()、onInactive()方法。

观察者变为活跃，就使用LiveData的dispatchingValue(observerWrapper)进行数据分发。
如果当前正在分发，则分发无效；
observerWrapper不为空，就使用considerNotify()通知真正的观察者，
observerWrapper为空 则遍历通知所有的观察者。

considerNotify:回调真正的mObserver的onChanged方法

setValue主线程更新数据
value赋值给mData，然后调用dispatchingValue(null)，遍历所有观察者 进行分发回调。


postValue子线程更新数据
把Runable对象抛到主线程，执行setValue
...
```
databinding:解决FindViewById 
布局中分数据层和常规布局
DataBinding会对元素为 <layout> 的布局文件进行预处理。 binding表达式 的view都被设置了 Tag。
DataBinding把最初布局文件中的<data>以及各个view中的 binding表达式 内容抽取出来，生成了一个名为 activtiy_main-layout.xml 文件。
这2个布局文件的tag位置可以对应起来。

DataBinding通过两个xml生成ActivityMainBinding和BR
ActivityMainBinding：继承自 ViewDataBinding，包含每个variable标签、每一个有id的View、每一个没有id但是处理中添加了Tag的View
BR：生成对应ActivityMainBinding类中变量的常量（一种标识符）。它对应一个会发生变化的数据，当数据改变后，你可以用该标识符通知 DataBinding，很快，DataBinding 就会用新的数据去更新UI。
...
#### 3.如果有一个任务需要线程池里所有任务执行完以后执行，应该怎么做
...
#### 6.手写单例，如果在不使用锁的情况下创建单例并保证单例安全性

静态内部类

#### 7.handler原理，子线程可以创建Handler吗？
...
JSBridge，addJavaScriptInterface
...
解决：通过reqestDisallowInterceptTouchEvent()方法能够设置mGroupFlag的值，从而调整if条件是否满足。
...
postdelay封装消息到Message.callback
...
https://blog.csdn.net/xhmj12/article/details/84918185
...
ANR，入队：根据时间排序，当队列满的时候，阻塞，直到用户通过next取出消息。当next方法被调用，通知MessagQueue可以进行消息的入队。
...
1.mTraversalScheduled保证同时间多次更改只会刷新一次
...
4.1默认开启VSYNC:调用scheduleVsyncLocked

申请VSYNC信号是在native方法里，接收回调是onVsync()中。
onVsync：使用mHandler发送异步msg，最终执行的就是doFrame()方法了。
...
#### 没有内存泄漏的情况下，应用占用内存比业内其他应用内存要多，怎么分析优化
...
#### 页面内有一些不需要每次都显示的view，怎么优化

ViewStub

#### recyclerview里面的图片显示速度比较慢，怎么优化
...
#### Handler 机制 ，如何处理发送延迟消息

#### 锁 DCL双重判断，为何第二次还要再判断
...
#### Glide 原理 Lrucache原理 Glide 是如何监控页面的生命周期

#### 我们需要写System .gc()方法吗？为什么
...
#### 问了launcher启动Activity的流程

#### application的oncreate方法可以被多次调用吗　　　　　　　　多进程技术有使用过吗
...
#### 注解有没有使用过 ， 使用过的场景 编译时注解和运行时注解的区别
