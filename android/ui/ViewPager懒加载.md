...
isViewCreated:View是否创建好了(如果没有创建好,去执行事件逻辑会报空指针)

mIsFristVisible:是否是第一次创建的标志位

currentVisibleState:当前的状态
...
getUserVisibleHint = true ，就去分发事件



### setUserVisibleHint:

在这个方法中去做懒加载的处理，因为和生命周期无关。



如果View已经创建好了,在去判断分发.(因为这个方法在生命周期之前会调用)

用 当前的状态 和 调用的状态来判断处理。
...
当用户按下home 或者 跳转新页面在跳转回来

需要判断

当前状态是不显示的(因为在pause分发了)

getUserVisibleHint 为true (因为默认在当前显示的页面按下的home)
...
getUserVisibleHint = true && !isHidden，就去分发事件(isHidden默认为false)
...
// 是能在负责分发事件的方法中判断一下当前父 fragment 是否可见，
// 如果父 fragment 不可见我们就不进行可见事件的分发
//查看父Fragment显示的情况，为空返回fasle，显示返回false，不显示 返回true
...
setOffScreenPageLimit(0)无效

默认为1：相当于不仅加载当前页，而且会加载后一页



ViewPager设置WrapContent无效

因为在计算子ChildView之前，已将自己的宽高计算好了
